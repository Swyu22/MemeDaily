...
> 用途：把本文件**整段**作为 Codex 自动化任务的提示词，Codex 即可在本地 Mac 上**完整接手**
> 「每日热梗」与「每日日报（新闻）」两条线的：选题研究 → 数据产出 → 校验 → 提交 → 推送上线。
...
`published_at` **你不用填**（发布步骤/兜底会盖真实推送时间；你只填 `generated_at`）。
...
### 6.2 优先题材（多民生、少「国家高光/政府色彩」）
民生服务/社会事件（高考出分与志愿填报、毕业季、暑运春运、开学、医保社保便民、天气生活提示；以及
**人人关心的重大社会事件，含地震/洪涝/台风等灾害**）、节日节气（母亲节/端午/五一/中秋/春节/清明/二十四节气）、
会展经济生活（广交会/进博会/车展/消费季/文旅出行/小店数字经济）、科技AI（国产大模型/AI应用；航天大工程
**适量别堆**）、科技向善凡人善举（公益/寻人/助农/救援互助/平民英雄）、文化非遗国潮、体育大赛。
...
`category`(民生社会|节日节气|国家高光|科技AI|科技向善|文化数字经济——**内部分类、读者看不到**，用于把控结构，
别让「国家高光」占太多)；`heat_rank`(整数，可见项**连续 1..N**，1=最热)；
...
# —— 去重锁：若 data/daily/<DATE>.json 或 data/daily-news/<DATE>.json 已 status:published，本线跳过 ——
# （可选）热梗预取：bash scripts/prefetch-hotlists.sh   # 生成 prefetch/*.txt 供你 Read，加速
# …研究 + 生成今天的 JSON（热梗写 data/daily/<DATE>.json；日报写 data/daily-news/<DATE>.json）…
npm run check                            # validate + validate:news + lint + typecheck + test + build，必须全过
# 只暂存今天这一个文件，绝不改以前的日子：
git add data/daily/<DATE>.json           # 或 data/daily-news/<DATE>.json
...
8. `git add 今天的文件` → commit（Conventional）→ `git pull --rebase` → `git push`。push 即触发部署。
...
1. **决定模式**（第 3 节）：Codex 唯一发布者（先停云端：清空 `CLAUDE_CODE_OAUTH_TOKEN` 或注释 schedule +
   停 cron-job.org），还是 Codex 兜底（cron 设在云端之后、靠去重锁补发）。**别两套都当主力。**
2. 新建两个 Codex 常开自动化（或一个跑两条线）：工作目录=上面的仓库路径；cron：日报 06:00、热梗 07:00
   （Asia/Shanghai；做唯一发布者时）。把**本文件整段**作为提示词。
3. 确保该机器：Codex 已登录、联网/浏览开启、git 对 `Swyu22/MemeDaily` 有 push 权限、装了 Node 22 且在仓库目录
   跑过 `npm ci`。
4. 不需要任何云端 secret / PAT——本地 push 用用户自己的 git 凭据，且 push 自动触发 Pages 部署。
