...
> v3 适配：本文是「人读的流程索引」。Claude Code 已把各步骤做成 slash 命令——
> 开工 `/start` · 交接 `/handoff` · 中途回写 `/sync` · 收尾 `/close`（按 tier 分级）。
> 其他模型仍按本文 + `AGENTS.md` + `ai/prompts/*` 执行。会话采用「一会话一文件」`YYYY-MM-DD--<slug>.md` 并登记 `ai/sessions/INDEX.md`。

## SOP-0：新建项目初始化（30–60 分钟）
> 一键走完「需求共创 → 固化状态 → 开工」：Claude Code 用 `/kickoff`，或把 `ai/prompts/PROJECT_KICKOFF_PROMPT.md` 发给任意模型。下面是其手动展开：
...
## SOP-3：每次收工（必须回写 · 按 tier 分级，Claude Code 用 `/close`）
1. 要求模型输出 Session Summary（Daily 或 Milestone）
2. 将总结写入 `ai/sessions/YYYY-MM-DD--<slug>.md` 并登记 `ai/sessions/INDEX.md`
