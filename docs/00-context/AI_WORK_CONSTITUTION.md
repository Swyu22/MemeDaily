...
## 5.5 分级回写（Ceremony Budget · V3）
> 治「仪式过重」：回写量随**改动规模**缩放，而非每次固定 7 步。tier 写在 `.cloud.md` frontmatter.tier。
> 不确定时运行 `bash scripts/checks/suggest-tier.sh worktree` 取建议。

| tier | 触发场景 | 必须回写 |
|------|----------|----------|
| `micro` | 改 1 行 / 配置 / typo | `.cloud.md`（updated + Last Done 一行）+ session 一行 |
| `feature` | 单模块功能 | micro + `current-iteration` checklist + Daily session |
| `milestone` | 跨模块 / 接口 / 架构 | feature + 关键文件头 + 模块 README + `PROJECT_MAP` + ADR + Milestone session + `sessions/INDEX.md` |

执行细节见 `ai/prompts/CLOSE_ROUTINE.md`（`/close` 据此分级，只做该级要求项）。
...
### Sync（回写 · 分级）
按 `.cloud.md` frontmatter.tier 取用清单（见 §5.5 与 `ai/prompts/CLOSE_ROUTINE.md`）：
- `micro`：`.cloud.md` + session 一行
- `feature`：+ plan checklist + Daily session
- `milestone`：+ 关键文件头 / 模块 README / `PROJECT_MAP` / ADR / Milestone session / `sessions/INDEX.md`
